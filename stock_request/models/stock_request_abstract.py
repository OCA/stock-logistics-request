...
class StockRequest(models.AbstractModel):
    ...
    # procurement_group_id was removed in Odoo v19
    # procurement.group model no longer exists
    # Group management is now handled internally by stock.picking
        ...
        for rec in self:
            if (
                ...
                raise ValidationError(
                    self.env._(
                        "You have entered a product that is assigned to "
                        "another company."
                    )
                )
            if (
    ...
    def _check_product_uom(self):
        ...
        for request in self:
            # In Odoo 19.0, UoM system changed:
            # - UoMs without relative_uom_id are reference UoMs
            # - UoMs with relative_uom_id reference another UoM
            # Two UoMs are compatible if they share the same reference UoM
            product_ref = (
                request.product_id.uom_id.relative_uom_id or request.product_id.uom_id
            )
            request_ref = (
                request.product_uom_id.relative_uom_id or request.product_uom_id
            )
            if product_ref != request_ref:
                raise ValidationError(
                    self.env._(
